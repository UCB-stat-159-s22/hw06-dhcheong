...
if whiten_data:
    ...
    strain_L1_whiten = u.whiten(strain_L1,psd_L1,dt)
...
template_p_whiten = u.whiten(template_p,psd_H1,dt)
template_c_whiten = u.whiten(template_c,psd_H1,dt)
...
for det in dets:
    ...
    template_whitened = u.whiten(template_rolled,interp1d(freqs, data_psd),dt)  # whiten the template
    ...
    if make_plots:
        ...
        # -- Plot the result
        u.plot_SNR(time, timemax, SNR, pcolor, eventname, det, plottype)
        u.plot_whitened(time, tevent, strain_whitenbp, pcolor, det, timemax, template_match, eventname, plottype)
        u.plot_ASD(template_fft,datafreq, d_eff, freqs, data_psd, pcolor, det, fs, eventname, plottype)
...
u.write_wavfile("audio/"+eventname+"_L1_whitenbp.wav",int(fs), strain_L1_whitenbp[indxd])

# re-whiten the template using the smoothed PSD; it sounds better!
template_p_smooth = u.whiten(template_p,psd_smooth,dt)
...
u.write_wavfile("audio/"+eventname+"_template_whiten.wav",int(fs), template_p_smooth[indxt])
...
strain_L1_shifted = u.reqshift(strain_L1_whitenbp,fshift=fshift,sample_rate=fs)

# write the files:
u.write_wavfile("audio/"+eventname+"_H1_shifted.wav",int(fs), strain_H1_shifted[indxd])
u.write_wavfile("audio/"+eventname+"_L1_shifted.wav",int(fs), strain_L1_shifted[indxd])

# and the template:
template_p_shifted = u.reqshift(template_p_smooth,fshift=fshift,sample_rate=fs)
u.write_wavfile("audio/"+eventname+"_template_shifted.wav",int(fs), template_p_shifted[indxt])
